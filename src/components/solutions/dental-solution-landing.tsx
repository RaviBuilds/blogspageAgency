      ...
      <section className="relative isolate overflow-hidden">
        ...
        <div className="mx-auto max-w-6xl px-6 pb-20 pt-20 lg:px-8 lg:pb-28 lg:pt-28">
          <div className="max-w-3xl">
            <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={0} className="mb-6 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-4 py-1.5 text-xs text-muted-foreground">
        ...
        <div className="mx-auto max-w-6xl px-6 lg:px-8">
          <FadeUp>
            <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
              {METRICS.map((m) => (<div key={m.label} className="rounded-xl border border-white/[0.08] bg-card p-6 text-center"><p className="text-4xl font-semibold tracking-tight text-primary">{m.value}</p><p className="mt-2 text-sm text-muted-foreground">{m.label}</p></div>))}
            </div>
      ...
      <section className="border-t border-white/[0.06] py-20 lg:py-24">
        <div className="mx-auto grid max-w-6xl gap-10 px-6 lg:grid-cols-[0.8fr_1.2fr] lg:px-8">
          ...
          <div className="grid gap-4">
            {PAIN_POINTS.map((point, i) => (<FadeUp key={point} delay={i * 0.08}><div className="flex gap-3 rounded-xl border border-white/[0.08] bg-card p-5 text-sm leading-relaxed text-muted-foreground"><TrendingDown className="mt-0.5 size-4 shrink-0 text-primary" /><span>{point}</span></div></FadeUp>))}
          </div>
        </div>
      </section>
      ...
      <section className="border-t border-white/[0.06] py-20 lg:py-24">
        <div className="mx-auto max-w-6xl px-6 lg:px-8">
          ...
          <motion.div variants={staggerParent} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} className="mt-12 grid gap-5 lg:grid-cols-2">
            <motion.div variants={staggerChild} className="relative overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.02] p-8 backdrop-blur-md">
            ...
            <motion.div variants={staggerChild} className="relative overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.02] p-8 backdrop-blur-md">
              ...
              <div className="relative">
                ...
                <ul className="mt-5 space-y-3">{TRANSFORMATION_WITH.map((item) => (<li key={item} className="flex gap-3 text-sm text-zinc-200"><CheckCircle2 className="mt-0.5 size-4 shrink-0 text-emerald-300" />{item}</li>))}</ul>
              </div>
            </motion.div>
          </motion.div>
          <FadeUp delay={0.12}>
        ...
        <div className="mx-auto max-w-6xl px-6 lg:px-8">
          ...
          <motion.div variants={staggerParent} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {BENEFITS.map((b) => { const Icon = b.icon; return (
              <motion.div key={b.title} variants={staggerChild} className="flex flex-col rounded-xl border border-white/[0.07] bg-white/[0.02] p-5 transition-colors hover:border-white/[0.14] hover:bg-white/[0.04]">
                <span className="flex size-10 shrink-0 items-center justify-center rounded-xl border border-white/[0.08] bg-primary/10"><Icon className="size-5 text-primary" /></span>
          ...
          <div className="mt-12 grid gap-4 md:grid-cols-2">
            <FadeUp>
              <article className="overflow-hidden rounded-xl border border-white/[0.08] bg-card transition-colors hover:border-white/[0.16]">
                <div className="border-b border-white/[0.06] bg-popover px-4 py-3"><div className="flex gap-1.5"><span className="size-2.5 rounded-full bg-white/15" /><span className="size-2.5 rounded-full bg-white/15" /><span className="size-2.5 rounded-full bg-white/15" /></div></div>
                <div className="relative aspect-[16/10] bg-[linear-gradient(135deg,var(--popover),var(--background))]" aria-hidden><div className="absolute inset-0 p-5"><div className="h-3 w-1/4 rounded-full bg-white/10" /><div className="mt-4 grid grid-cols-7 gap-1">{Array.from({ length: 14 }).map((_, i) => (<div key={i} className={`h-6 rounded border border-white/[0.06] ${i === 4 || i === 9 ? "bg-primary/20" : "bg-white/[0.03]"}`} />))}</div><div className="mt-4 h-8 w-1/3 rounded-lg border border-primary/30 bg-primary/10" /></div></div>
            ...
            <FadeUp delay={0.08}>
              <article className="overflow-hidden rounded-xl border border-white/[0.08] bg-card transition-colors hover:border-white/[0.16]">
                <div className="border-b border-white/[0.06] bg-popover px-4 py-3"><div className="flex gap-1.5"><span className="size-2.5 rounded-full bg-white/15" /><span className="size-2.5 rounded-full bg-white/15" /><span className="size-2.5 rounded-full bg-white/15" /></div></div>
                <div className="relative aspect-[16/10] bg-[linear-gradient(135deg,var(--popover),var(--background))]" aria-hidden><div className="absolute inset-0 p-5"><div className="h-3 w-1/3 rounded-full bg-white/10" /><div className="mt-4 grid grid-cols-3 gap-3"><div className="h-16 rounded-lg border border-white/[0.06] bg-white/[0.03]" /><div className="h-16 rounded-lg border border-white/[0.06] bg-white/[0.03]" /><div className="h-16 rounded-lg border border-white/[0.06] bg-primary/15" /></div><div className="mt-3 h-20 rounded-lg border border-white/[0.06] bg-white/[0.02]" /></div></div>
              ...
              <div className="relative grid gap-10 lg:grid-cols-[1.1fr_0.9fr]">
                <div>
                  ...
                  <blockquote className="mt-6 border-l-2 border-primary/40 pl-4">
                    ...
                    <footer className="mt-2 text-xs text-muted-foreground">— {CASE_STUDY.testimonial.author}, {CASE_STUDY.testimonial.role}{CASE_STUDY.testimonial.placeholder && (<span className="ml-2 rounded bg-white/[0.06] px-1.5 py-0.5 text-[10px] uppercase tracking-wider text-muted-foreground/50">Placeholder</span>)}</footer>
                  </blockquote>
                  <div className="mt-6 flex flex-wrap gap-2">{CASE_STUDY.stack.map((t) => (<span key={t} className="rounded-full border border-white/[0.1] bg-white/[0.03] px-3 py-1 text-xs text-foreground/80">{t}</span>))}</div>
                </div>
                <ul className="grid content-start gap-3">{CASE_STUDY.outcomes.map((o) => (<li key={o} className="flex gap-3 rounded-xl border border-white/[0.08] bg-popover p-4 text-sm text-muted-foreground"><Check className="mt-0.5 size-4 shrink-0 text-primary" /><span>{o}</span></li>))}</ul>
              </div>
            </div>
          </FadeUp>
      ...
      <section className="border-t border-white/[0.06] py-20 lg:py-24">
        <div className="mx-auto max-w-6xl px-6 lg:px-8">
          ...
          <ol className="mt-12 flex flex-col">
            {TIMELINE.map((phase, i) => (<FadeUp key={phase.window} delay={i * 0.06}><li className="relative flex gap-6 border-l border-white/[0.1] pb-8 pl-6 last:pb-0"><span className="absolute -left-[5px] top-1.5 size-2.5 rounded-full bg-primary" /><div><p className="text-xs font-medium uppercase tracking-[0.18em] text-primary">{phase.window}</p><h3 className="mt-1 text-lg font-medium tracking-tight">{phase.title}</h3><p className="mt-1 text-sm leading-relaxed text-muted-foreground">{phase.detail}</p></div></li></FadeUp>))}
          </ol>
        </div>
      </section>
          ...
          <div className="mx-auto max-w-6xl px-6 lg:px-8">
            <motion.div variants={reveal} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }}>
              ...
              <ul className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {relatedPosts.map((post) => (<li key={post._id}><Link href={`/blogs/${post.slug}`} className="group flex h-full flex-col rounded-xl border border-white/[0.08] bg-white/[0.02] p-5 transition-colors hover:border-white/[0.16]"><p className="text-xs text-muted-foreground">{post.publishedAt ? new Date(post.publishedAt).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" }) : null}</p><h3 className="mt-2 font-medium tracking-tight transition-colors group-hover:text-primary">{post.title}</h3></Link></li>))}
              </ul>
            </motion.div>
          </div>
        </section>
